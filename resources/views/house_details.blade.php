			...
			<!--CONTENT CONTAINER-->
				...
				<div id="content-container">
					...
					<!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
					<div id="page-title">
						<h1 class="page-header text-overflow">House Details</h1>
						...
						<div class="panel-body">
						  <div class="table-responsive">
							<table class="table table-striped table-bordered" cellspacing="0" width="100%">
								...
								<tbody>
									@foreach($house_detail_not_varify as $index=>$value)
									<tr>
										<td>{{($house_detail_not_varify->currentpage()-1) * $house_detail_not_varify->perpage() + $index + 1 }}</td>
										...
										<td>
										@if($user_access[0]->fn_update=='Yes')
										<a class="label label-success" href="#" onclick="confirmData('{{$value->personal_details_id}}');">Verify</a> 
									    <a class="label label-danger" href="#" onclick="RejectData('{{$value->personal_details_id}}');">Reject</a>
										@endif
										</td>
										</tr>
									@endforeach
			...
			function confirmData(id)
			{
				...
				if(flag)
				{
					var url="{{url('/')}}/varifyHousedata?id="+id;
			...
			function RejectData(id)
			{
				...
				if(flag)
				{
					var url="{{url('/')}}/rejectHousedata?id="+id;
					window.location.assign(url);
				}
			}
			</script>
