...
<div class="dashboard-main-body">
  ...
  <h6 class="font-semibold mb-0 dark:text-white">Mange User</h6>
  <ul class="flex items-center gap-[6px]">
    ...
    <li class="font-medium dark:text-white">User List</li>
  </ul>
</div>
    <div class="grid grid-cols-12">
      <div class="col-span-12">
        ...
        class="form-select form-select-sm w-auto dark:bg-dark-2 dark:text-white border-neutral-200 dark:border-neutral-500" 
        onchange="filterByStatus()">
              ...
              <a href="{{url('/')}}/createUser" class="btn btn-sm text-white bg-primary-600 hover:bg-primary-700 flex items-center gap-2"><i class="ri-add-line"></i> Create User</a>
              @endif
            </div>
          </div>
          <div class="card-body">
              ...
              <table class="table bordered-table mb-0">
                ...
                <tbody>
                  ...
                  <tr>
					...
					<a href="#" onclick="confirminActive('{{$value->id}}&status=In-Active');" class="bg-success-100 dark:bg-success-600/25 text-success-600 dark:text-success-400 px-6 py-1.5 rounded-full font-medium text-sm" title="click here to In-Active user">Active</a>
					@endif
					@if($value->status=='In-Active')
					<a href="#" onclick="confirmActive('{{$value->id}}&status=Active');" class="bg-danger-100 dark:bg-danger-600/25 text-danger-600 dark:text-danger-400 px-6 py-1.5 rounded-full font-medium text-sm" title="click here to Active user">In-Active</a>
					@endif
					@endif   
					</td>
					<td>
					...
					@endif
					@if($user_access[0]->fn_update=='Y')
					<a href="{{url('/')}}/updateUser/{{$value->id}}" class="w-8 h-8 bg-success-100 dark:bg-success-600/25 text-success-600 dark:text-success-400 rounded-full inline-flex items-center justify-center"><iconify-icon icon="lucide:edit"></iconify-icon></a>
...
function filterByStatus() {
    ...
    window.location.href = "{{ url('/manageUser') }}?status=" + status;
}
</script>
