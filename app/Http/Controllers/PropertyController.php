...
{
    public function PropertTypeList()
    {
        ...
        $user_access = $menu = DB::table('user_access_type')
            ...
            ->where([
                ...
                ['user_access_type.sub_menu', 'Property Type'],
            ])->get();
        //dd($user_access);
        ...
        if (session()->get('user_type') == 'Admin') {
            $property_type_name = DB::table('property_type')->where('isdeleted', 'N')->paginate(10);
        } else {
            $property_type_name = DB::table('property_type')->where('isdeleted', 'N')->paginate(10);
        }
        return View('list_property_type')->with('menu', $menuData)->with('user_access', $user_access)->with('property_type_name', $property_type_name);
    }

    public function PropertType(Request $request)
    {
        if (session()->get('id') == null) {
            return redirect('login');
        }
        ...
        $menuData = app('App\Http\Controllers\DashboardController')->MenuList();
       $data = array(
           'property_type_name' => $request->get('property_type_name'),
           'tax_rate' => $request->get('tax_rate'),
            'created_at' => date('Y-m-d H:i:s'),
           );
        $insertedid=DB::table('property_type')->insertGetId($data);
        return redirect('/Property-Type')->with('message','Property Type Added Successfully.');

    }
    public function DeletePropertType($id)
    {
        ...
        DB::table('property_type')->where('id', $id)->delete();

        
        return back()->with('message','Property Type Deleted Successfully.');
    }

    public function UpadtePropertTypeView(Request $request)
    {
        if (session()->get('id') == null) {
            return redirect('login');
        }
        ...
        $id       = $request->get('id');
        if (session()->get('user_type') == 'Admin') {
            $property_type_name = DB::table('property_type')->where('id', $id)->get();
        } else {
            $property_type_name = DB::table('property_type')->where([['id', $id]])->get();
        }
        //  dd($asset_name);
        return View('edit_property_type')->with('menu', $menuData)->with('user_access', $user_access)->with('property_type_name', $property_type_name);
    }
    public function SaveUpdatePropertType(Request $request)
    {
        if (session()->get('id') == null) {
            return redirect('login');
        }
        ...
        // dd($data);
        if (session()->get('user_type') == 'Admin') {
            DB::table('property_type')->where('id', $id)->update($data);
        } else {
            DB::table('property_type')->where([['id', $id]])->update($data);
        }
        // dd($id);
        return redirect('/Property-Type')->with('message','Property Type Updated Successfully.');
    }
